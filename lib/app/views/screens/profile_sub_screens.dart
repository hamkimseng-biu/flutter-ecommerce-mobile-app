            ...
            child: StreamBuilder<QuerySnapshot>(
              ...
              builder: (ctx, snapshot) {
                ...
                if (snapshot.hasData && snapshot.data!.docs.isNotEmpty) {
                  ...
                  // Fallback to mock
                  orders = List<Map<String, dynamic>>.from(_mockOrders);
                }
            ...
            child: Padding(
              ...
              child: Row(
                children: [
                  ...
                  if (status == 'Delivered' && _activeTab == 4)
                    TextButton.icon(
                      onPressed: () {
                        // Navigate to first product's detail for review
                        final firstItem = (o['items'] as List).firstOrNull;
                        if (firstItem != null) {
                          Get.toNamed(
                            AppRoutes.productDetail,
                            arguments: firstItem['name'],
                          );
                        }
                      },
                      icon: const Icon(
...
class _AddressesScreenState extends State<AddressesScreen> {
  ...
  Future<void> _loadAddresses() async {
    ...
    // Fallback mock if Firestore empty
    setState(() {
      _addresses = [
        {
          'firestoreId': 'mock1',
          'recipient': 'Seng Hamki',
          'phone': '+855 12 345 678',
          'label': 'Home',
          'street': '123 Cluck Street, Hen House District',
          'city': 'Phnom Penh',
          'province': 'Phnom Penh',
          'zip': '12000',
          'isDefault': true,
        },
        {
          ...
          'label': 'Office',
          'street': '456 Feather Lane, Suite 200',
          'city': 'Siem Reap',
          'province': 'Siem Reap',
          'zip': '17000',
          'isDefault': false,
  ...
  void _showAddressDialog({int? index}) {
    ...
    final recipientCtrl = TextEditingController(
      text: (existing as Map<String, dynamic>?)?['recipient'] ?? '',
    );
    final phoneCtrl = TextEditingController(
      text: (existing as Map<String, dynamic>?)?['phone'] ?? '',
    );
    final labelCtrl = TextEditingController(
      text: (existing as Map<String, dynamic>?)?['label'] ?? '',
    );
    final streetCtrl = TextEditingController(
      text: (existing as Map<String, dynamic>?)?['street'] ?? '',
    );
    final cityCtrl = TextEditingController(
      text: (existing as Map<String, dynamic>?)?['city'] ?? '',
    );
    final provinceCtrl = TextEditingController(
      text: (existing as Map<String, dynamic>?)?['province'] ?? '',
    );
    final zipCtrl = TextEditingController(
      text: (existing as Map<String, dynamic>?)?['zip'] ?? '',
    );
    var isDefault =
        (existing as Map<String, dynamic>?)?['isDefault'] == true ||
        _addresses.isEmpty;
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () async {
                ...
                final addr = {
                  'firestoreId': isEdit ? existing!['firestoreId'] ?? '' : '',
                ...
                // Save to Firestore
                try {
                  if (isEdit && (addr['firestoreId'] as String).isNotEmpty) {
  ...
  Future<void> _loadCards() async {
    try {
      ...
      setState(() {
        ...
        _bankAccounts = banks.isNotEmpty ? banks : _mockBanks();
        _loading = false;
      });
    } catch (_) {
      setState(() {
        _cards = _mockCards();
        _bankAccounts = _mockBanks();
        _loading = false;
      });
    }
  }
      ...
      body: ListView(
        ...
        children: [
          ...
          _tile(
            'Currency',
            _currency,
            () => _selectOption('Currency', _currency, [
              'USD (\$)',
              'KHR (៛)',
              'EUR (€)',
              ...
              onPressed: () {
                Get.dialog(
                  AlertDialog(
                    ...
                    content: const Text(
                      'This will clear your local cache and preferences.',
                      style: TextStyle(fontSize: 14),
                    ),
                    actions: [
                      TextButton(
